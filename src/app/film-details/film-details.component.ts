...
export class FilmDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.filmService
      .getFilm(selectedMovieUrl.slice(this.individualUrlSubstringIndex))
      .subscribe((result) => {
        this.film = result;
  ...
  getCharacter(tableIndex: number): void {
    if (this.charactersTable[tableIndex].name == null) {
      const charLink = this.charactersTable[tableIndex].characterLink.slice(
        this.individualUrlSubstringIndex
      );
